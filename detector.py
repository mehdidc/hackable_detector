...
class Detector(nn.Module):
    def __init__(
        ...
        layers,
        scales,
        branch_layer_index_for_scale,
        aspect_ratios_for_scale,
        nb_classes,
        nb_coords=4,
        ...
        self.bounding_box_prediction_layers = nn.ModuleList(
            [
                prediction_layer_func(
                    layers[layer_index].out_channels, nb_coords * len(aspect_ratios)
                )
                for layer_index, aspect_ratios in zip(
        ...
        self.class_prediction_layers = nn.ModuleList(
            [
                prediction_layer_func(
                    layers[layer_index].out_channels, nb_classes * len(aspect_ratios)
                )
                for layer_index, aspect_ratios in zip(
                    branch_layer_index_for_scale, aspect_ratios_for_scale
                )
    ...
    def predict(self, X):
        ...
        boxes_list = []
        classes_list = []
        for boxes, classes, aspect_ratios in zip(
            boxes_for_scale, classes_for_scale, self.aspect_ratios_for_scale
        ):
            ...
            classes = classes.view(
                batch_size, len(aspect_ratios), self.nb_classes, h, w
            )
