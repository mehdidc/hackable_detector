...
def build_anchors_matrix(scales, per_scale_aspect_ratios, per_scale_feature_maps, offset=0.5):
    boxes = build_anchors_grid_per_scale(scales, per_scale_aspect_ratios, per_scale_feature_maps, offset=offset)
    boxes = [boxes_for_scale.reshape((-1, 4)) for boxes_for_scale in boxes]
    return np.concatenate(boxes, axis=0)

def build_anchors_grid_per_scale(scales, per_scale_aspect_ratios, per_scale_feature_maps, offset=0.5):
    boxes = []
    for scale, aspect_ratios, feature_map_dim in zip(scales, per_scale_aspect_ratios, per_scale_feature_maps):
        ...
        for i, aspect_ratio in enumerate(aspect_ratios):
            ...
            boxes_for_scale[:, :, i, XMIN] = (
                x + offset) / feature_map_dim
            boxes_for_scale[:, :, i, YMIN] = (
                y + offset) / feature_map_dim
