...
def prediction_layer(in_channels, out_channels):
    seq = nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0))
    return seq
...
def main(*, images_folder, annotations_file):
    ...
    filenames, annotations = get_annotations(
        images_folder=images_folder, annotations_file=annotations_file
    )
    ...
    detector = builder.build(
        nb_classes=nb_classes, prediction_layer_func=prediction_layer
    )
    anchors = build_anchors_matrix_from_detector(
        detector, input_shape=(3, image_size, image_size)
    )
    ...
    for epoch in range(nb_epochs):
        for batch_index, (images, encoded_boxes, classes, filenames) in enumerate(
            dataloader
        ):
            t0 = time.time()
            images = images.cuda()
            ...
            loc_loss = (
                smooth_l1_loss(pred_boxes[pos], encoded_boxes[pos], size_average=False)
                / nb_pos
            )
            pred_classes, classes = imbalance_strategy(pred_classes, classes)
            class_loss = (
                class_loss_func(pred_classes, classes, size_average=False) / nb_pos
            )
            ...
            print(
                "Epoch {:05d}/{:05d} Iter {:05d} Batch {:05d}/{:05d} Loss : {:.3f} Loc : {:.3f} "
            ...
            if nb_iter % 10 == 0:
                ...
                true_boxes = decode_bounding_boxes(anchors, true_boxes)
                ...
                true_classes = [
                    dataset.decode_class[class_id] for class_id in true_classes
                ]
                pred_classes = [
                    dataset.decode_class[class_id] for class_id in pred_classes
                ]
